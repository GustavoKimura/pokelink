...
import type { GameRunState } from "../models/Map";
...
export const useGameStore = create<GameStore>((set) => ({
  ...
  setPlayer: (player) =>
    set((state) => ({ runState: { ...state.runState, player } })),
  startRun: () =>
    set((state) => {
  ...
  completeNode: (nodeId) =>
    set((state) => {
      ...
      const updatedNodes = state.runState.mapNodes.map((node) => {
        ...
        if (!node.completed) return { ...node, unlocked: false };
        return node;
      });
      return {
        runState: {
  ...
  updatePlayer: (player) =>
    set((state) => ({ runState: { ...state.runState, player } })),
  setRunPhase: (phase) =>
    set((state) => ({ runState: { ...state.runState, runPhase: phase } })),
